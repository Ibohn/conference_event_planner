...
const ConferenceEvent = () => {
  ...
  const handleToggleItems = () => {
    // DEBUG: on trace le toggle et le total calculé au moment d’ouvrir/fermer
    console.log("handleToggleItems called -> toggling details panel");
  ...
  // -------- Sélection des items pour l’affichage
  ...
  // ---------- DEBUG GLOBAL : totaux + items (mets ces logs avant le return)
  ...
  console.log(
    "TOTAL attendu:",
    venueTotalCost + avTotalCost + mealsTotalCost
  );
  ...
  items.forEach((item) => {
    if (item.type === "meals") {
      console.log(
        `${item.name} x ${numberOfPeople} = ${item.cost * numberOfPeople}`
      );
    } else {
      console.log(`${item.name} x ${item.quantity} = ${item.cost * item.quantity}`);
    }
  });

  // ---------- Composant d’affichage du tableau (garde ton log existant)
    ...
    <>
      <navbar className="navbar_event_conference">
        ...
        <div className="left_navbar">
          ...
          {/* DEBUG: on passe par handleToggleItems pour tracer le clic */}
